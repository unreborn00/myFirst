...
for i in range(n):
    try:
        product_tiles = wait.until(EC.presence_of_all_elements_located((By.CSS_SELECTOR, "div[class='product-tile']")))
        product_tile = product_tiles[i]

        product_name = product_tile.find_element(By.XPATH, ".//a[@class='link lll-font-weight-medium']").text
        product_price = product_tile.find_element(By.CSS_SELECTOR, "span[data-lll-pl = 'price']").text
